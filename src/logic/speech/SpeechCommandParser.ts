...
> = [
  [
    /\b(zurücksetz(?:en|e)?|zuruecksetz(?:en|e)?|reset(?:te)?|neu(?:start(?:en)?)?|restart(?:ing)?)\b/,
    "reset",
  ],
  [/\b(stop|stopp(?:en)?|anhalten|abbrechen|halt|cancel)\b/, "stop"],
  [/\b(vorles(?:en|e)?|sprich|ansag(?:en|e)?|read(?:ing)?|speak)\b/, "speak"],
  [
    /\b(berechn(?:en|e)?|ergebnis(?:se)?|auswert(?:en|e)?|calculate|result)\b/,
    "calculate",
  ],
];
function coordinatesAfter(
  text: string,
  matcher: RegExp,
  until?: RegExp,
): Coordinates | undefined | "incomplete" {
...
export function parseSpeechCommand(rawText: string): SpeechCommand {
  const text = rawText
    ...
    .replace(/[^\p{L}\p{N},.-]+/gu, " ")
    .trim();
  for (const [matcher, type] of actionWords)
    if (matcher.test(text)) return { type };
  ...
  if (target === "incomplete" && parseNumbers(text).length === 0)
    return { type: "select-coordinates", subject: "target" };
  if (artillery === "incomplete" && parseNumbers(text).length === 0)
    return { type: "select-coordinates", subject: "artillery" };
  if (artillery === "incomplete")
    return {
      type: "incomplete",
      message: "Bitte nenne X- und Y-Koordinate der Artillerie.",
    };
  if (target === "incomplete")
    return {
      type: "incomplete",
      message: "Bitte nenne X- und Y-Koordinate des Ziels.",
    };
  if (artillery || target)
    return { type: "set-coordinates", artillery, target };
  if (targetWords.test(text))
    return { type: "select-coordinates", subject: "target" };
  if (artilleryWords.test(text))
    return { type: "select-coordinates", subject: "artillery" };
  const values = parseNumbers(text);
  if (values.length >= 2)
    return { type: "coordinates", coordinates: { x: values[0], y: values[1] } };
  return { type: "unknown" };
}
