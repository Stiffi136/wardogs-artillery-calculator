...
function App() {
  ...
  const [status, setStatus] = useState('🎤 Sprachsteuerung AUS'), [notice, setNotice] = useState<string | null>(null), [speechOn, setSpeechOn] = useState(false), [autoSpeak, setAutoSpeak] = useState(false), [modelLoading, setModelLoading] = useState(false), [pendingCoordinates, setPendingCoordinates] = useState<PendingCoordinates>(null), [audioLevel, setAudioLevel] = useState(0), [vadActive, setVadActive] = useState(false)
  ...
  const speak = useCallback(() => { const currentSolution = solutionRef.current; if (!currentSolution) { setNotice('Bitte vollständige, gültige Koordinaten eingeben.'); return }; output.current.speak(`Entfernung ${fmt(currentSolution.distanceKilometers)} Kilometer. Richtung ${Math.round(currentSolution.azimuthDegrees)} Grad.`) }, [])
  ...
  const handleText = useCallback((text: string) => {
    ...
    if (command.type === 'select-coordinates') {
      ...
      setStatus('✓ Befehl erkannt'); feedback.current.success(); setNotice(solutionRef.current ? 'Feuerlösung aktualisiert.' : 'Bitte X und Y für Artillerie und Ziel vollständig eingeben.')
      ...
      output.current.stop(); setStatus('✓ Befehl erkannt'); setNotice('Vorlesen gestoppt.'); feedback.current.success()
    } else {
