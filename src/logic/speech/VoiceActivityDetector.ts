...
export interface VadOptions { speechThreshold?: number; silenceDurationMs?: number }
export class VoiceActivityDetector {
  ...
  process(audio: Float32Array, sampleRate: number): 'speech-start' | 'speech-end' | null {
    ...
    if (rms >= this.threshold) { this.silenceMs = 0; if (this.state === 'IDLE') { this.state = 'SPEAKING'; return 'speech-start' }; return null }
    if (this.state === 'SPEAKING' && (this.silenceMs += duration) >= this.silenceDurationMs) { this.state = 'IDLE'; this.silenceMs = 0; return 'speech-end' }
    return null
  }
  reset() { this.state = 'IDLE'; this.silenceMs = 0 }
}
